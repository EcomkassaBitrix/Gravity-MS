...
$path = str_ireplace('/vendor-endpoint.php/api/moysklad/vendor/1.0/apps/', '', $path);
...
switch ($method) {
    case 'PUT':
        ...
        $appUid = $data->appUid;
        $accessToken = $data?->access[0]?->access_token ?? null;
        $additional = $data?->additional ?? null;

        $id = $additional?->fiscalApi?->id;
        $token = $additional?->fiscalApi?->token;

        $rsaData = new RsaData();
        $rsaData->setAppId($appUid);
        $rsaData->setId($id);
        $rsaData->setToken($token);

        $rsaStorage = new RsaFileStorage();
        $rsaStorage->persist($rsaData);
