...
Обязательно разрешите запросы: `GET`, `POST`, `PUT`, `DELETE`
...
Выполните `vendor/bin/doctrine-migrations migrate` для создания изменения в базе данных. Если есть новые миграции, то выполнять команду каждый раз после развертывания в приложении.
...
Файл для хранения логов вебхуков (не должна быть доступна по прямому URL, должна быть доступна для записи):

```
WEBHOOK_LOG=../logs/webhook.log
```

Файл для хранения логов запросов vendor (не должна быть доступна по прямому URL, должна быть доступна для записи):

```
VENDOR_LOG=../logs/vendor.log
```
...
Идентификатор решения (appId) для модерации  (получать в личном кабинете разработчика):

```
TEST_APP_ID=
```

Глобальный идентификатор (appUid) для модерации (получать в личном кабинете разработчика):
...
Секретный ключ для модерации (получать в личном кабинете разработчика):

```
TEST_SECRET_KEY=
```

Драйвер PDO, например: pdo_pgsql
